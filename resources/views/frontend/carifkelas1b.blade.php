...
<form class="" action="/carifkelas1b" method="GET" style="margin-right:190px ;">
        ...
        <div class='overflow-x-auto w-full'>
            ...
            <table class='mx-auto max-w-4xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                ...
                @foreach($fkelas1b as $fb)
                    <tr>
                        <td class="px-6 py-4">
                        
                            <div class="flex items-center space-x-3">
                                <div class="inline-flex w-10 h-10"> <img class='w-10 h-10 object-cover rounded-full' alt='User avatar' src='/image/{{$fb->image}}' /> </div>
                                <div>
                                    <p>{{$fb->nama}}</p>
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-4 text-center">{{$fb->msoffice}}</td>
                        <td class="px-6 py-4 text-center">{{$fb->photoshop}}</td>
                        <td class="px-6 py-4 text-center">{{$fb->laravel}}</td>
                        @endforeach
                    </tr>
                </tbody>
            </table>
